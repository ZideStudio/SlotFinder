...
func NewRouter() *gin.Engine {
	...
	v1 := router.Group("/v1")
	{
		...
		accountGroup := v1.Group("/account")
		{
			...
			accountGroup.PATCH("/avatar", guard.AuthCheck(&guard.AuthCheckParams{RequireAuthentication: true, RequireCompleteProfile: false}), guard.MaxUploadSizeMiddleware(10<<20), accountRouter.UploadAvatar)
