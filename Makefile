...
start:
	...
	@(cd front && mise exec -- sh -c 'npm install && npm run start') & (cd back && mise exec -- go tool air); wait

# Start frontend only (installs frontend dependencies)
front:
	@echo "\n📱 Starting frontend on https://localhost"
	@cd front && mise exec -- sh -c 'npm install && npm run start'

# Start backend only with hot reload
back:
	@echo "\n🔧 Starting backend on https://localhost/api"
	@cd back && mise exec -- go tool air
