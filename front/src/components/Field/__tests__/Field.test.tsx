...
describe('Field', () => {
  it('registers the input with react-hook-form', () => {
    ...
    expect(screen.getByRole('textbox', { name: 'Email' })).toHaveAttribute('name', 'user.email');
  });
    ...
    const WrapperWithError = () => {
      ...
      return (
        <FormProvider {...methods}>
          <Field input={MockInput} name="user.email" label="Email" />
          <button onClick={() => setError('user.email', { message: 'Invalid email' })}>Trigger error</button>
        </FormProvider>
      );
    };
